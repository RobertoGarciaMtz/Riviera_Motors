...
{
    public class TbLeyAntilavadoController : Controller
    {
        // GET: HomeController1
        public ActionResult Index()
        {
            var context = HttpContext.RequestServices.GetService(typeof(riviera_businessContext)) as riviera_businessContext;
            var list = context.TbLeyAntilavado.ToList();
            foreach (TbLeyAntilavado ti in list)
            {
        ...
        public ActionResult Create(TbLeyAntilavado a)
        {
            try
            {
                var context = HttpContext.RequestServices.GetService(typeof(riviera_businessContext)) as riviera_businessContext;
                context.TbLeyAntilavado.Add(a);
                context.SaveChanges();
        ...
        public ActionResult Edit(int id)
        {
            ...
            ViewBag.Datospersona = context.TbDatosPersona.Select(dp => new Microsoft.AspNetCore.Mvc.Rendering.SelectListItem { Text = dp.Curp, Value = dp.IdDatosPersona.ToString() });
            ViewBag.Estado = context.CEstados.Select(es => new Microsoft.AspNetCore.Mvc.Rendering.SelectListItem { Text = es.Descripcion, Value = es.IdEstados.ToString() });
            if (context.TbLeyAntilavado.Where(le => le.IdDocumentos == id).First() is TbLeyAntilavado e)
            {
                return View(e);
            }
            return NotFound();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(int id, TbLeyAntilavado a)
        {
            try
            {
                var context = HttpContext.RequestServices.GetService(typeof(riviera_businessContext)) as riviera_businessContext;
                var objectEdit = context.TbLeyAntilavado.FirstOrDefault(te => te.IdDocumentos == a.IdDocumentos);
                if(objectEdit != null)
                {
                    ...
                    context.TbLeyAntilavado.Update(objectEdit);
                    context.SaveChanges();
                }
                return RedirectToAction(nameof(Index));
            }
